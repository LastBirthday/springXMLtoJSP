...
<% List names = (List)request.getAttribute("customersNames"); %>
<% Iterator iterator; %>
<% Iterator iterator2; %>
...
<c:if test="${!empty customersNames}">
    <div class="container">
        <table class="table table-bordered table-hover">
            <tr>
                <th class="text-center"><h3>Customers names:</h3></th>
            </tr>
            <% for (iterator = names.iterator(); iterator.hasNext(); ) { %>
            <tr>
                <td class="text-center"><%=iterator.next()%></td>
            </tr>
            <% } %>
        </table>
    </div>
